...
{
	return single_open(file, dma_proc_show, proc_get_parent_data(inode));
}
...
{
	return single_open(file, ch_proc_show, proc_get_parent_data(inode));
}
...
{
	return single_open(file, log_proc_show, proc_get_parent_data(inode));
}

static ssize_t log_level_write(struct file *file, const char __user *data, size_t len, loff_t *off)
{
	struct dw_dma *dw = proc_get_parent_data(file_inode(file));
	char buf[3];
