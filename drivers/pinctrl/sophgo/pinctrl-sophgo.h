...
struct sg_soc_pinctrl_data {
	const struct pinctrl_pin_desc *pins;
	unsigned int npins;
	struct sg_group *groups;
	int groups_count;
	const struct sg_pmx_func *functions;
	int functions_count;
	struct class *p_class;
};

int sophgo_pinctrl_probe(struct platform_device *pdev);
int sophg0_pmux_probe(struct platform_device *pdev);
